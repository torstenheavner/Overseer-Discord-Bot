...
async def on_ready():
	eou.log(text="Ready")
	game = discord.Activity(type=discord.ActivityType.listening, name="your DMs.")
...
async def ping(ctx):
	await ctx.send(embed=eou.makeEmbed(title="Pong!", description="Overseer is online."))
	eou.log(text="Bot Pinged", ctx=ctx)


with open("T:/all 2/tokens/overseer.txt", "r") as token:
	bot.run(token.read())
